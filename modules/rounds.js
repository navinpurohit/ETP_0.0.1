...
console.log("round.js:scope.round- "+scope.round);
	library.db.query(sql.summedRound, { round: scope.round, activeDelegates: constants.activeDelegates }).then(function (rows) {
		...
		rows[0].rewards.forEach(function (reward) {
			...
			console.log("round.js: reward"+reward);
		});
...
console.log("round.js:scope.roundFees "+scope.roundFees+",scope.roundRewards :"+scope.roundRewards+",scope.roundDelegates:"+scope.roundDelegates);
